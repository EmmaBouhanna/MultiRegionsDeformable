...
def get_contour_eps_levels(mask, eps=1e-15):
    ...
    v1 = abs(P1)/d
    v2 = abs(P2)/d
    Ah = ((np.vstack((P, np.zeros([1, n]))) +
          np.vstack((np.zeros([1, n]), P))) > 0)
    Vh = np.maximum(np.vstack((v1, np.zeros([1, n]))), np.vstack(
        (np.zeros([1, n]), v2)))
    ...
    v1 = abs(P1)/d
    v2 = abs(P2)/d
    Av = ((np.hstack((P, np.zeros([p, 1]))) +
          np.hstack((np.zeros([p, 1]), P))) > 0)
    Vv = np.maximum(np.hstack((v1, np.zeros([p, 1]))), np.hstack(
        (np.zeros([p, 1]), v2)))
