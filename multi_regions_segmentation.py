...
def get_params():
    ...
    parser.add_argument('--resize', type=int, default=200,
                        help="Size to which transform the image")
    parser.add_argument('--initialization', default="circles",
                        help="Choose initalization method (between circles and kmeans)")
    parser.add_argument('--n_regions_init', type=int, default=6,
                        help="Number of regions for kmeans/circles horizontally")
    parser.add_argument('--n_iter', type=int, default=200,
                        help="Number of iterations (max)")
    parser.add_argument('--mu', type=float, default=1,
                        help="Regularization param mu")
    parser.add_argument('--dt', type=float, default=1, help="Time step")
    parser.add_argument('--dilat_size', type=int,
                        default=3, help="Dilation factor")
    parser.add_argument('--thresh', type=float, default=.1,
                        help="Threshold for region merging")
    parser.add_argument('--it_view', type=int, default=10,
                        help="Interval to plot results")
    parser.add_argument('--save_video', required=False,
                        help="If given, will save the video to required path")
...
def initialize_regions_circles(I, n_circles=6):
    ...
    for i in range(0, n_circles):
        for j in range(0, n_circles):
            regions = cv2.circle(regions, ((n*i)//n_circles + n//(n_circles*2), (n*j)//n_circles + n//(
                n_circles*2)), n//(n_circles*2 + 2), color=it_regions, thickness=-1)
            it_regions += 1
    return regions
...
if __name__ == '__main__':
    ...
    # Initialize Phi
    Phi = perform_fast_marching_isotropic(
        np.ones_like(I_dilat), dilat_size*contours.T)
    ...
    for it in range(args.n_iter):
        ...
        contours_dilat, contours_regions_dilat = get_contours_regions(
            regions_dilat)

        # Get Voronoi regions from C_ij
        unique_neighbors = np.unique(contours_regions, axis=0)
        Voronoi, _ = get_voronoi_cij(
            contours_dilat, contours_regions_dilat, unique_neighbors, shape_vor=I_dilat.shape)

        # Compute new Phi (thanks to extended velocity)
        Phi_new, norm_grad_phi = update_Phi(
            I_dilat, Phi, g, regions_dilat, unique_regions, Voronoi, unique_neighbors, dt, ci, mu)
        ...
        # STEP 2
        # eps-level sets
        eps_level_sets, global_ls_mask, mask_per_reg = get_eps_levels(
            regions_dilat, unique_regions, Phi_new, eps_ls)

        # new contours from eps level sets
        regions_new, Indices, Indices_dilated = new_contours_from_eps_levels(
            eps_level_sets, I_dilat, unique_regions, dilat_size)

        # Update contours
        contours, contours_regions = get_contours_regions(regions_new)

        # merge neighboring regions that are too similar
        regions_new = merge_regions(
            regions_new, contours_regions, ci, thresh_merge)
